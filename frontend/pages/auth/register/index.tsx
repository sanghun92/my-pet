...
import { useLogin, useRegister, useSendEmailVerificationCode } from '@/core/hooks/useAuthHooks';
...
import { BackDropLoading } from '@/core/components/loading';
...
const RegisterPage = () => {
  ...
  const onSubmit = async (state: MemberRegisterState) => {
    ...
    await registerMember(state)
      .then(() => sendEmailVerificationCode(state.email))
      .then(() => login(state))
      .then(() => {
